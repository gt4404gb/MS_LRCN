...
def create_model(Config):
    ...
    if Config.SELF_SUPERVISED_METHOD == 'lrcn':
        ...
        encoder = TransformerEncoder(
            input_dim=Config.INPUT_DIM, latent_dim=Config.LATENT_DIM, d_model=128,
...
def data_augment_1d(x, mask_ratio=0.1, noise_level=0.01):
    """在 GPU 上进行 1D 数据增强，优化后的向量化实现"""
    device = x.device
    batch_size, feature_dim = x.shape
    num_mask = int(feature_dim * mask_ratio)

    # 生成与 x 同形状的随机数矩阵
    rand = torch.rand((batch_size, feature_dim), device=device)
    # 对每个样本取出 num_mask 个随机位置（索引）
    indices = rand.topk(num_mask, dim=1).indices  # shape: [batch_size, num_mask]

    # 构建一个布尔型掩码矩阵，初始全为 False
