...
if __name__ == '__main__':
    ...
    B, C, d = 8, 16, 5  # Batch size, number of channels, and spatial dimensions
    input_tensor = torch.randn(B, C, d, d)  # Generate random input tensor

    # Initialize the BDC module
    is_vec = True
    input_dim = (C, d, d)
    dimension_reduction = 16  # Dimension reduction
    activate = 'relu'

    bdc_module = BDC(is_vec=is_vec, input_dim=input_dim, dimension_reduction=dimension_reduction, activate=activate)

    # Perform a forward pass
    output = bdc_module(input_tensor)

    # Print output shape and details
